...
class projectColor(Node):
    ...
    def build_colored_pc(self, x, y, z, label, pc_range):
        ...
        for i in range(pc_range):
            ...
            # if it encounters a new label, pick new color for next group of 30
            if int(label[i]) != nowlabel:
                nowlabel = int(label[i])
                j = 0
                rgb = self.pixelpick((x[i] - self.xdiff), (y[i] - self.ydiff), (z[i] - self.zdiff))

            # once 30 points has been reached of the same color within the current object, select new color
            if j > 30:
                j = 0
                rgb = self.pixelpick((x[i] - self.xdiff), (y[i] - self.ydiff), (z[i] - self.zdiff))
